...
public class SocketBroker extends AbstractBroker implements EventBroker {
    ...
    public void start() {
        if (server==null) {
            ...
            coordinator.schedule(SynchronizedAgentFactory.build(Action.DISCARD_OBSOLETE_CONNECTIONS, 
                    PriorityLock.HIGH_ISOLATED, Frequency.PERIODIC, ()->discardObsoleteClients())
                    .delayed(5000).every(2000));
            
            logger.debug("{}: Creating SocketServer", getClass().getSimpleName());
            this.server = new SocketServer(this, 
                config.getBrokerServerConnectionHandlerThreads(), 
                getShardId().getInetPort(), 
                getShardId().getInetAddress().getHostAddress(), 
                config.getFollowerUseNetworkInterfase(), 
                coordinator,
                config.getBrokerRetryDelayMs(), 
    ...
    private boolean post(
        ...
        if (client == null) {
            logger.info("{}: ({}) CREATING SocketClient for Shard: {}", getClass().getSimpleName(), 
                    getShardId(), channel.getAddress());
            this.clients.put((DirectChannel)channel, client = new SocketClient(
                    channel, coordinator, 
                    config.getBrokerRetryDelayMs(), config.getBrokerMaxRetries(), 
                    getShardId().toString(), config));
        }
        ...
        return client.send(new MessageMetadata(event, channel.getChannel().getName()));
    }
