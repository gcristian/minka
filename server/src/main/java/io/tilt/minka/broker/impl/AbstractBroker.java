...
public abstract class AbstractBroker extends ServiceImpl implements EventBroker, Consumer<MessageMetadata> {
	...
	public void accept(MessageMetadata meta) {
		logger.debug("{}: ({}) Consuming {}", getClass().getSimpleName(), shardId, meta.getPayloadType().getSimpleName());
	...
	public final boolean subscribeEvent(
	    ...
	    try {
            ...
            final String key = channel.getChannel().getName() + eventType.getSimpleName();
            ...
            if (drivers!=null && drivers.contains(consumer)) {
            	...
            	logger.info("{}: ({}) Subscribing channel: {} with Type: {} with Driver: {} ", 
            		getClass().getSimpleName(), shardId, channel.getChannel().getName(),
            		eventType.getSimpleName(), consumer.getClass().getSimpleName());
            }
            
            Collection<String> channeles = channelsPerConsumer.get(consumer); 
            if (channeles != null) {
                channeles = channeles.stream()
                        .filter(i->i.equals(channel.getFullName()))
