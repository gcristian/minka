...
public class PartitionTable {
    ...
    public enum ClusterHealth {        
        /* visibility: if there was no shard changing status
         * working: theere's work-load balance among the nodes 
         */ 
        STABLE,
        /* visibility: if there was at least one shard changing status 
         * that will provoke a duty reallocation
         * working: there's a reallocation in progress 
         */
        UNSTABLE,
        ;           
    }
    ...
    public enum ClusterCapacity {
        IDLE,
        NORMAL,
        /* if there's only one Shard alive, 
         * or workload calculation needs more working shards
         * */        
        INSUFFICIENT,        
    }
    ...
    public String toString() {
        ...
        .append("Shards: ").append(shardsByID.size())
        .append(" Crud Duties: ").append(this.crud.size());
    ...
    public void addShard(final Shard shard) {
        if (this.shardsByID.containsKey(shard.getShardID())) {
            logger.error("{}: Inconsistency trying to add an already added Shard {}", 
        ...
        if (duty.getDutyEvent().is(EntityEvent.ASSIGN) || duty.getDutyEvent().is(EntityEvent.CREATE)) {
            for (Shard sh : partitionsByShard.keySet()) {
                if (!sh.equals(where) && partitionsByShard.get(sh).getDuties().contains(duty)) {
                    ...
                    .append(" tries to Report Duty: ").append(duty)
                    .append(" already in ptable's Shard: ").append(sh).toString();
    ...
    public List<Shard> getShardsByState(final ShardState filter) {
        return shardsByID.values().stream()
                .filter(i -> i.getState() == filter || filter == null)
    ...
    public void logStatus() {
                ...
                if (partition == null) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("{}: Status for Shard: {} = Weight: {} with {} Duties: [ {}]", 
                                getClass().getSimpleName(), shard, partition.getWeight(), 
                                partition.getDuties().size(), partition.toString());
                    } else {
                        logger.info("{}: Status for Shard: {} = Weight: {} with {} Duties", 
                                getClass().getSimpleName(), shard, partition.getWeight(), 
                                partition.getDuties().size());
                    }
        ...
        if (!this.missing.isEmpty()) {
            logger.warn("{}: {} Missing duties: [ {}]", getClass().getSimpleName(), 
                    missing.size(), this.missing.toString());
        }
        if (!this.dangling.isEmpty()) {
            logger.warn("{}: {} Dangling duties: [ {}]", getClass().getSimpleName(), 
                    dangling.size(), this.dangling.toString());
        }
