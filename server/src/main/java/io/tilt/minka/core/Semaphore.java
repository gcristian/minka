...
import static io.tilt.minka.business.Semaphore.Action.BOOTSTRAP;
import static io.tilt.minka.business.Semaphore.Action.DISTRIBUTOR_PERIODIC;
import static io.tilt.minka.business.Semaphore.Action.HEARTBEAT_REPORT;
import static io.tilt.minka.business.Semaphore.Action.INSTRUCT_DELEGATE;
import static io.tilt.minka.business.Semaphore.Action.LEADERSHIP;
import static io.tilt.minka.business.Semaphore.Action.PARTITION_TABLE_UPDATE;
import static io.tilt.minka.business.Semaphore.Action.SHEPHERD_PERIODIC;
import static io.tilt.minka.business.Semaphore.Action.SHUTDOWN;
import static io.tilt.minka.business.Semaphore.Action.STUCK_POLICY;
...
public interface Semaphore extends Service {
     ...
     */
    enum Hierarchy {
        /* doesnt lock related actions
         * TODO wait some time and interrupt them on expiration, then allow depender to run */
        PARENT, 
        /* allowed to run if related actions are unlocked 
         * then lock/released them altogether with depender or wait them to be released if locked */
        SIBLING,
        /* doesnt lock anything, this allows the action to run when related is locked */ 
        CHILD,
        ;
    }
    
    /* an action's hierarchy relation to other actions later translated to tasks, permissions, retries, etc */
    interface Rule {
        Action getAction();
    ...
    enum Action {
        ...
        SHUTDOWN(Scope.LOCAL), 
        DISTRIBUTE_ENTITY(Scope.LOCAL),  
        LEADERSHIP(Scope.GLOBAL), 
        /* both to avoid inconsistent behaviour and to simplify synchronization of shared data 
         * both modify the partition table from different methods*/ 
        DISTRIBUTOR_PERIODIC(Scope.LOCAL), 
        SHEPHERD_PERIODIC(Scope.LOCAL),
        /* we cannot hope for the PartitionDelegate to be thread-safe 
         * this include Take, Release & Update: they mutually exclude-'emselves one another */
        INSTRUCT_DELEGATE(Scope.LOCAL),
        /* shard's enforcement to take rights on an assigned duty */
        RESERVE_DUTY(Scope.GLOBAL),
        /* in order to avoid heartbeat intoxication with concurrent delegate instructions */
        ...
        CLUSTER_COMPLETE_SHUTDOWN(Scope.GLOBAL),
        CLUSTER_COMPLETE_REBALANCE(Scope.GLOBAL),
        
        /* for hierarchy matters - an action that represent all others to the one compared */ 
        ANY(Scope.LOCAL),
    ...
    default List<Rule> getLockingRules() {
        ...
        rules.add(RuleFactory.builder(BOOTSTRAP).add(PARENT, Arrays.asList(ANY)));
        rules.add(RuleFactory.builder(SHUTDOWN).add(PARENT, Arrays.asList(ANY)));
        
        // At Leader's
        rules.add(RuleFactory.builder(LEADERSHIP).add(CHILD, Arrays.asList(Action.BOOTSTRAP)));
        rules.add(RuleFactory.builder(SHEPHERD_PERIODIC)
                .add(SIBLING, Arrays.asList(DISTRIBUTOR_PERIODIC, PARTITION_TABLE_UPDATE)));
        rules.add(RuleFactory.builder(DISTRIBUTOR_PERIODIC)
                .add(SIBLING,Arrays.asList(SHEPHERD_PERIODIC, PARTITION_TABLE_UPDATE))); 
        rules.add(RuleFactory.builder(PARTITION_TABLE_UPDATE)
                .add(SIBLING,Arrays.asList(SHEPHERD_PERIODIC, DISTRIBUTOR_PERIODIC)));
        
        // At Follower's
        rules.add(RuleFactory.builder(INSTRUCT_DELEGATE).add(SIBLING, Arrays.asList(HEARTBEAT_REPORT)));
        rules.add(RuleFactory.builder(HEARTBEAT_REPORT).add(SIBLING, Arrays.asList(STUCK_POLICY, INSTRUCT_DELEGATE))); 
        rules.add(RuleFactory.builder(STUCK_POLICY).add(SIBLING, Arrays.asList(HEARTBEAT_REPORT)));
