...
public class Bootstrap extends ServiceImpl {
		...
		*/
		...
		public Bootstrap(final Config config, final ConfigValidator validator, final SpectatorSupplier spectatorSupplier,
			...
			this.readyAwareBooting = scheduler.getAgentFactory().create(Action.BOOTSTRAP_BULLETPROOF_START,
						PriorityLock.HIGH_ISOLATED, Frequency.ONCE_DELAYED, () -> readyAwareBooting())
		...
		private void bootLeadershipCandidate() {
			if (config.bootstrapPublishLeaderCandidature()) {
				logger.info("{}: ({}) Candidating Leader (lap: {})", getName(), shardId,
