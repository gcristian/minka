...
public interface Semaphore extends Service {
	...
	enum Action {
		...
		DISCARD_OBSOLETE_CONNECTIONS(Scope.LOCAL), FOLLOWER_POLICIES_CLEARANCE(Scope.LOCAL), BROKER_INCOMING_MESSAGE(
				Scope.LOCAL), BROKER_CLIENT_START(Scope.LOCAL), BROKER_SERVER_START(
						Scope.LOCAL), BOOTSTRAP_BULLETPROOF_START(
								Scope.LOCAL), BOOTSTRAP_LEADERSHIP_CANDIDATURE(Scope.GLOBAL),
