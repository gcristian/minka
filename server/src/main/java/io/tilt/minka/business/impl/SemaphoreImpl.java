...
public class SemaphoreImpl extends ServiceImpl implements Semaphore {
    ...
    private Permission checkState(final Action action) {
        if (action.getScope()==Scope.LOCAL) {
            ...
            if (lock.isHeldByCurrentThread()) {
                logger.error("{}: {} lock already acquired by YOU ! CHECK YOUR CODE !", 
     ...
     */
    private boolean lockRelated(
            final Action cause, 
            final boolean threadLock, 
    ...
    private boolean lock(
        ...
        switch (action.getScope()) {
            case GLOBAL:
                return locks.acquireDistributedLock(nameForDistributedLock(action, null), 
                        true, DISTRIBUTED_LOCK_WAIT_MS, TimeUnit.MILLISECONDS);
            case LOCAL:
